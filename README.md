...
| **notrest** | The **establishment + continuation verb** — writes `COORD.md` plus a marker-delimited v2 protocol block in `AGENTS.md` (Codex), `CLAUDE.md` (Claude), or both explicitly. Idempotent, atomic, byte-safe, root-contained; `--surface auto|codex|claude|both`; check and continuation remain read-only. |
...
- **UserPromptSubmit** — two cheap per-prompt lines: the COORD reminder (append one honest ledger line when this prompt's work lands), and the **router** — a task-shaped prompt gets one line naming the suite's verb for it (`/notrest:<skill>`, fine to skip deliberately; silent otherwise).
