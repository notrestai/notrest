...
> **v4.8.0 distribution note (2026-09-06):** notrest is **part of Atlas** and ships to Atlas
> customers from v4.8.0. The repo is private; an install needs repository access *and* an
> owner-issued key at `~/.notrest/access-key` (or `NOTREST_ACCESS_KEY`), without which every
> hook stays silent but for one SessionStart line. The gate covers the harness on a machine,
> not the source files — stated plainly because a gate oversold is a gate that misleads.
> Versions through v4.7.1 were released under the MIT License and remain so; v4.8.0 onward is
> proprietary (`LICENSE`).
...
upgrades: 1) close the guard hole (identical duplicate lines passed the agent-id guard; race fixed at root in 3.6.1, guard still latent — chip open). 2) record duration + tool-call count when the payload carries them: zero-cost speed-law instrumentation per lane.
...
### agentswarm
does (v4.1.0): `scripts/swarm.py report [--window Nd] [--json]` — the decomposition gauge. Joins spend receipts + COORD-AGENTS + briefs into per-lane rows (calls/secs derived from transcripts for old receipts), bands them GREEN/WIDE/MONOLITH against this estate's own measured numbers, and pairs size with rework so over-decomposition is visible too. Exits 0/5/6; window anchored on the newest receipt, so runs are reproducible. Fixture: swarm-fixture.sh (30 asserts).
