...
| **ROSTER PARITY** | every skill *dir* is **named** on every surface a reader browses — a **table row** in the root `README.md` (whose bold cell is the dir name), and a mention in the inner `README.md`, the marketplace entry description and `docs/TUTORIAL.md`. FAILs listing the absent names per source | **the three invisible skills, v4.6.1.** `beam`, `mentor` and `tieredswarm` shipped with no README row and no mention anywhere, for five weeks, and every gate passed — because SKILL COUNT compares a *number* to a number, and a README that **says** thirty-two while **listing** twenty-nine is arithmetically perfect. Counting is not naming. A README with no roster table at all is held to the naming bar instead of the row bar, because inventing a convention is not a gate's job. |
...
| **ESTATE** | `COORD.md` header + parseable ledger lines, the `COORD-AGENTS.md` machine header, `spend/ledger.md` through `spend.py report`, `compile/candidates.json` as valid JSON — and, since 4.7.0, one **detail line** for the learnings store (`learnings: N records, newest <age>`), reported beside the other ledgers because a lessons store nobody has written to in a month is a fact about the estate the owner should be able to see without running a second tool. It is a **detail, not a status**: doctor does not judge how many lessons an estate ought to have, and the check's PASS/WARN never turns on it | recap, compile and graph all *parse* these files. A damaged header does not break the file — it breaks every reader downstream, quietly. `spend.py report` exiting **4** is not a doctor failure: it is the routing gate firing correctly, reported as WARN with the verdict. |
| **LOOP HEALTH** | whether the estate's learning loop is actually turning: **uncited triggers** (a lesson lived and never banked), **untested admissions with no `open` record behind them**, **open questions by age** (nobody closed them), **learnings per week**, **records a lane proposed that no seat has accepted or rejected**, and **candidates drafted or proposed but never decided**, read from `compile/candidates.json`. It PASSes while the loop is turning and **WARNs when it stops — it never FAILs**, because none of these is a broken install and a quiet week is not a defect; the check exists so a loop that has stopped turning is visible before anyone notices it as a feeling | **the loop that only measured itself, 4.7.0.** The learnings loop shipped with a gate at each end and no instrument in the middle: an estate could bank nothing for a month, leave four questions open past their recheck dates and draft three candidates it never adopted, and every check stayed green — because each one was individually fine. A loop with no health reading is a loop you find out about from a defect |
...
| **TOKEN BUDGET** | the always-on context cost the CLI reports for this plugin, against a ceiling of **5200 tok** (`ALWAYS_ON_CEILING` in `doctor.py` — the live figure is in the check's own line) | every skill description is loaded into **every** session whether the skill fires or not, so the harness taxes work it never does. The number is read from `claude plugin details` — the CLI's figure, never doctor's estimate — and the check SKIPs honestly when no CLI or no id will answer, rather than guessing. |
| **GITIGNORE** | `/graph/` and `/compile/` present *and anchored*; the skills' own `graph/` and `compile/` dirs not ignored; **and no derived output TRACKED under `plugins/*/graph/` or `plugins/*/compile/`** (any `*.json`/`*.html` git knows about there) | **the unanchored gitignore, v2.16.0**, then its mirror image, **v4.6.1**. First `graph/` without the leading slash matched at *any* depth and un-tracked `plugins/notrest/skills/graph/` — the shipped plugin was missing a skill and `git status` stayed clean about it. Then the anchored fix left the other half open: a scan run from *inside* the plugin wrote `plugins/notrest/graph/{graph,river}.{html,json}`, 125 KB of derived output the root anchor never covered, and it rode in every consumer's install for five weeks referenced by nothing. One rule keeps the source tracked; this one keeps the derived output out. |
| **RENDER SURFACES** | the version stamps in `docs/oracle-skill-flow.html` match `plugin.json`, **and the page's own `N-skill` phrase matches the skill-dir count** (digits or spelled, e.g. `31-skill` / `thirty-one-skill`) | a rendered doc is a shipped surface. A stale stamp ships a lie to whoever opens it — and so does a stale **count**: v4.6.1's render said "31-skill harness" beside 32 skill dirs and passed, because the check pinned only the version. A number in prose drifts exactly like a version stamp, so it is gated the same way. A page that states no count is reported as not-stated, never guessed. |
